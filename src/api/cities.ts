  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (keywords.join(",").includes("dijon")) {
        reject(
          new Error(
            "Failed to calculate distance due to 'dijon' presence in cities passed"
          )
        );
      } else if (keywords.join(",").includes("fail")) {
        reject(
          new Error(
            "Failed to calculate distance due to 'fail' presence' in cities passed"
          )
        );
      } else {
        if (keywords.length === 0) {
          ...
          keywords.forEach((keyword) => {
            const results = cities.filter(
              (city) => city.name.toLowerCase() === keyword.toLowerCase()
            );
