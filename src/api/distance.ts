  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (intermidiateCities.length === 0) {
        ...
        var firstDistance: number = calculateDistance(
          origin,
          intermidiateCities[0]
        );
        distanceResponse.distanceBtnCities.push({
          city1: origin,
        ...
        if (intermidiateCities?.length !== 1) {
          ...
          for (var i: number = 0; i < intermidiateCities.length - 1; i++) {
            var intermidiateCityRouteDistance: number = calculateDistance(
              intermidiateCities[i],
              intermidiateCities[i + 1]
            );
            distanceResponse.distanceBtnCities.push({
        ...
        var lastDistance: number = calculateDistance(
          destination,
          intermidiateCities[intermidiateCities.length - 1]
        );
