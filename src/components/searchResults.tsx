...
const SearchResults: React.FC<Props> = (props) => {
  ...
  const [smallTableComponents, setSmallTableComponents] = React.useState<
    smallTableCopmponent[]
  >([
    {
      desc: "Origin",
    ...
    },
    {
      desc: "Destination",
      value:
        searchResults?.distanceBtnCities[
          searchResults.distanceBtnCities.length - 1
        ]?.city2?.name,
    },
    {
      desc: "Departure",
  ...
  React.useEffect(() => {
    ...
    var intermidiateCitiesParam = String(
      params.get("intermidiate_cities")
    )?.split(",");
    const usingParameters = async () => {
      ...
      await getCities(citiesInvolved)
        .then((data) => {
          validCities = data;
        })
        .catch((error) => {
      ...
      const intermidiateCities = validCities.filter(
        (city) =>
  ...
  return loading ? (
    <div className="flex flex-col min-w-full justify-center items-center pt-5 space-y-3">
      <CircularProgress />
      <p>Loading</p>
    </div>
  ) : error ? (
    <div className="flex flex-col min-w-full justify-center items-center pt-5 space-y-5">
      <p className="text-[150px] font-bold text-red-500">Oops!</p>
      <p>{error?.message}</p>
      <Link to={"/"}>
        <button className="bg-purple-900 p-2  px-5 text-white  rounded-3xl text-lg">
    ...
    <div className=" flex w-full p-5 min-h-screen justify-center items-center bg-white">
      <div className="flex w-full md:w-[800px] min-h-[650px] space-y-10 p-5 rounded-md flex-col">
        <h1 className="text-3xl font-extralight text-purple-900">
          Your Flight Details
        </h1>
        ...
        <div className="flex  bg-purple-50 flex-col w-full space-y-4">
          <div className="flex p-5 flex-row justify-between w-full">
            <h5 className="w-fit text-[50px] font-bold mt-10 text-purple-900">
              {searchResults?.distanceBtnCities[0]?.city1.name}
            </h5>
            <img
              src={"airplane.gif"}
              className="w-52 h-40 object-contain rounded-md "
            />
          ...
          <div className="flex flex-row p-5 justify-between">
              ...
              <div className="flex flex-row">
                ...
                <div className="flex flex-col space-y-2">
                  <p className="text-gray-500 text-sm">
                    :{" "}
                    {searchResults?.date.substr(
                      searchResults?.date.length - 12
                    )}
                  </p>
            ...
            <div className="flex flex-col space-y-2">
              {searchResults?.distanceBtnCities.map((distanceDesc) => (
                <div className="flex flex-row   ">
                  <p className="w-40 mr-2 text-gray-500 text-sm">
                    {distanceDesc?.city1?.name} - {distanceDesc?.city2?.name}
                  </p>
                  <p className="text-gray-500 text-sm">
                    : &nbsp;{distanceDesc?.routeDistance} Km
                  </p>
                </div>
              ))}
            </div>
            <div className="flex flex-col">
              <div className="flex flex-row">
                ...
                <div className="flex flex-col space-y-2">
                  ...
                  <p className="text-gray-500 text-sm">
                    : {searchResults?.distanceBtnOriginAndDestination} Km{" "}
                  </p>
          ...
          <div className="flex flex-row w-full border-t border-black">
            {smallTableComponents.map((component, index) => (
              ...
              >
                <div
                  className={`flex  justify-center items-center h-10  w-full text-purple-900  border-black   border-b font-bold`}
                >
                  {component.desc}{" "}
                </div>
                <div className="flex  justify-center items-center h-10 w-full text-sm">
                  {component.value}
                </div>
              </div>
            ))}
